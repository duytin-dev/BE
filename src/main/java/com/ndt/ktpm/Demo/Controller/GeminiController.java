...
public class GeminiController {
	...
	public ResponseEntity<?> chat(@RequestBody GeminiPromptRequest body) {
		...
		try {
			...
			HttpStatus st = HttpStatus.resolve(e.getStatusCode());
			if (st == null) {
				st = HttpStatus.BAD_GATEWAY;
			}
			return ResponseEntity.status(st).body(Map.of("error", e.getMessage()));
	...
	@GetMapping("/ai/gemini/models")
	public ResponseEntity<?> listGeminiModels() {
		try {
			return ResponseEntity.ok(Map.of("generateContentModelIds", geminiService.listGenerateContentModelIds()));
		} catch (GeminiUpstreamException e) {
			HttpStatus st = HttpStatus.resolve(e.getStatusCode());
			if (st == null) {
				st = HttpStatus.BAD_GATEWAY;
